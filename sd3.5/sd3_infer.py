...
HEIGHT = 1024
# Pick your prompt
PROMPT = "This is a F1 race car. Change its color to green."
...
def main(
    ...
    **kwargs,
    ...
    assert 0 <= shard_index < num_shards, f"shard_index must be in [0, {num_shards-1}]"
    ...
    tmp_root = Path(out_dir) / "_tmp_edits"
    ...
    rows = [r for i, r in enumerate(all_rows) if i % num_shards == shard_index]
    total = len(rows)
    print(f"[INFO] 分片 {shard_index}/{num_shards}：本进程将处理 {total} 条样本")

    local_seed = (seed or 0) + int(shard_index)

    def _find_image_for_id(_id: str):
        ...
        for pat in patterns:
            g = glob.glob(os.path.join(images_dir, pat))
            if g:
                return g[0]
        return None
    ...
    for r in rows:
        _id = str(r["ID"]).strip()
        edit_prompt = str(r["edit_prompt"]).strip()

        src_img = _find_image_for_id(_id)
        if src_img is None:
            print(f"\033[33m[WARN] 找不到原图：{_id}（跳过）\033[0m")
            skipped += 1
            continue

        dst_path = Path(target_dir) / f"{_id}_edited.png"
        if dst_path.exists():
            print(f"[INFO] 已存在，跳过：{dst_path}")
            processed += 1
            continue

        tmp_dir = tmp_root / f"{_id}"
        ...
        try:
            ...
            gen_files = sorted(
                [p for p in Path(tmp_dir).glob("**/*") if p.is_file()],
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )
            if not gen_files:
                print(f"\033[31m[ERR] 未在 {tmp_dir} 找到生成文件：{_id}\033[0m")
                skipped += 1
                continue
            ...
            for p in gen_files:
                if p.suffix.lower() in [".png", ".jpg", ".jpeg", ".webp"]:
                    newest = p
                    break
            newest = newest or gen_files[0]

            dst_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(newest), str(dst_path))
            ...
            print(f"\033[92m[OK] {_id} -> {dst_path.name}\033[0m")
            processed += 1

        except Exception as e:
            print(f"\033[31m[ERR] 生成失败 {_id}: {e}\033[0m")
            skipped += 1
            continue

    print(f"[DONE] 成功 {processed} / {total}，跳过 {skipped}")
