...
model_path = ""  # TODO: Download from https://huggingface.co/ByteDance-Seed/BAGEL-7B-MoT
...
max_mem_per_gpu = (
    "40GiB"  # Modify it according to your GPU setting. On an A100, 80 GiB is sufficient to load on a single GPU.
)
...
def main(args):
    ...
    for row in tqdm(df.itertuples(), total=len(df)):
        try:
            ...
            output_dict["image"].save(str(output_path))

        except Exception as e:
            print(f"Error processing {row.ID}: {e}")


argparser = argparse.ArgumentParser(description="Bagel Edit Inference Script")
...
argparser.add_argument(
    ...
    type=int,
    default=50,
    help="Number of diffusion timesteps to use during inference.",
)
argparser.add_argument(
    "--cfg_text_scale",
    type=float,
